			...
			<div class="tbl-header" >
				<table>
					<thead>
						<tr>
							<th>
							<select name="subject" onchange="location.href=this.value" > <!-- style="vertical-align:middle; margin:auto; text-align-last:center;" -->
							<option value="">카테고리</option>
							<option value="BoardNotice.jsp">공지사항</option>
							<option value="BoardTH.jsp">조별게시판</option>
							<option value="BoardFree.jsp">자유게시판</option>
							<option value="BoardSuggestion.jsp">건의게시판</option>
							</select>
							</th>
							<th>번호</th>
							<th>제목</th>
					...
					<tbody>
						<c:forEach items="${boardList }" var="list">
							<tr>
								<td>${list.subject }</td>
								<td>${list.num}</td>
								<td><a href="writeDetail?num=${list.num}&loginnick=${nick}&level=${Level}" style="color:black;">${list.title }</a></td>
								<td>${list.nick}</td>
								<td>${list.date}</td>
								<td>${list.hit}</td>
								<c:if test="${Level eq 'M' }">
